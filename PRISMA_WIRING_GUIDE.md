...
-
- This document serves as a reference for the Prisma code patterns used
- in the database wiring. The implementation is complete in the codebase.
-
...
const now = new Date();
const expiryDeadline = new Date(now.getTime() + withinDays _ 24 _ 60 _ 60 \* 1000);
...
(cert.expiryDate.getTime() - now.getTime()) / (24 _ 60 _ 60 \* 1000),
);
return {
id: cert.id,
crewId: cert.crewId,
...
status: 'VALID',
expiryDate: {
lte: new Date(Date.now() + 30 _ 24 _ 60 _ 60 _ 1000),
},
},
include: { crew: true },
});
...
alerts,
metrics: {
safeManningCompliance: vessels.length > 0 ? (compliantVessels / vessels.length) _ 100 : 100,
